@props(['permissions', 'label' => null, 'type' => 'primary', 'action' => ''])
...
@else
    @if ($action == 'button')
        <button type="button"
            {{ $attributes->merge(['class' => 'flex justify-center items-center gap-1 w-fit font-medium rounded text-sm px-5 py-2 disabled:opacity-75 ' . $bgColorClass]) }}>{{ $label ?? $slot }}</button>
    @else
        <a
            {{ $attributes->merge(['class' => 'flex justify-center items-center gap-1 w-fit font-medium rounded text-sm px-5 py-2 disabled:opacity-75 ' . $bgColorClass]) }}>{{ $label ?? $slot }}</a>
    @endif
@endif
